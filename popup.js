class ProducerPopup {
  ...
  async importRules(file) {
    try {
      ...
      const oldRules = this.rules.slice(); // keep the in-memory snapshot before overwriting
      this.rules = importedRules;
      await chrome.storage.local.set({ rules: importedRules });

      // Update popup state
      await this.saveState("import", oldRules); // passing oldRules to compare and reload affected tabs properly
      this.updateUI();
