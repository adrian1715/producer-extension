...
class ProducerBlockedPage {
    ...
    async applyThemeAndText() {
        ...
        const primaryColor = this.sanitizeHexColor(data.blockPagePrimaryColor, "#667eea");
        const accentColor = this.sanitizeHexColor(data.blockPageAccentColor, currentTheme.accentColor);
        document.body.style.background = this.buildBackground(currentTheme.background, primaryColor, data.blockPageBackgroundImage || "");
        ...
        if (container) {
            ...
            container.style.boxShadow = `0 24px 48px ${this.hexToRgba(primaryColor, 0.24)}`;
        }
        ...
        const showActionButtons = data.blockPageShowActionButtons !== undefined
            ? data.blockPageShowActionButtons
            : true;
        const showQuotes = data.blockPageShowQuotes !== undefined ? data.blockPageShowQuotes : true;
        if (titleEl) {
        ...
        warningButtons.forEach((button) => {
            const warningBackground = data.theme === "blackwhite"
                ? "rgba(255, 255, 255, 0.14)"
                : this.hexToRgba(primaryColor, 0.2);
            const warningBorder = data.theme === "blackwhite"
    ...
    applyFaviconLinks(faviconHref, fallbackFaviconHref = "", replaceExisting = false) {
        if (!faviconHref)
            return;
    ...
    restorePreviousFavicon() {
        if (!this.isUsableOriginalFavicon(this.originalFavIcon))
            return;
    ...
    async updateFocusedTime() {
        try {
            ...
            const totalSessionTimeFormatted = this.formatTime(response.totalSessionTime);
    ...
    updateBlockTypeUI() {
        ...
        const statsEl = document.querySelector(".stats");
        if (addExceptionBtn)
            addExceptionBtn.style.display = show;
        if (undoPermanentRuleBtn)
            undoPermanentRuleBtn.style.display = hide;
        if (blockNumberEl)
            blockNumberEl.style.display = show;
        if (statsEl)
            statsEl.style.display = show;
    }
