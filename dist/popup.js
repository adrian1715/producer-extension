...
class ProducerPopup {
    constructor() {
        ...
        this.sessionStartTime = null; // Timer anchor while focus mode is active
        this.isLoaded = false;
    ...
    initializeElements() {
        ...
        this.ruleType = this.$("ruleType");
        this.addRuleBtn = this.$("addRule");
        ...
        this.paramKeyInput = this.$("paramKeyInput");
        this.paramValueInput = this.$("paramValueInput");
        this.paramInputsContainer = this.$("paramInputsContainer");
        this.addParamRuleBtn = this.$("addParamRule");
        ...
        this.ruleSetNameInput = this.$("ruleSetNameInput");
        ...
        this.modeGrayscaleToggle = this.$("modeGrayscaleToggle");
        ...
        this.modeGrayscaleToggleView = this.$("modeGrayscaleToggleView");
        ...
        this.blockPageTitle = this.$("blockPageTitle");
        this.blockPageMessage = this.$("blockPageMessage");
        this.blockPageShowQuotes = this.$("blockPageShowQuotes");
        this.blockPageImage = this.$("blockPageImage");
        ...
        this.blockPageImageFileInput = this.$("blockPageImageFileInput");
        this.blockPageImageStatus = this.$("blockPageImageStatus");
        this.blockPagePrimaryColor = this.$("blockPagePrimaryColor");
        this.blockPagePrimaryColorDisplay = this.$("blockPagePrimaryColorDisplay");
        this.blockPagePrimaryColorText = this.$("blockPagePrimaryColorText");
        this.blockPageAccentColor = this.$("blockPageAccentColor");
        this.blockPageAccentColorDisplay = this.$("blockPageAccentColorDisplay");
        this.blockPageAccentColorText = this.$("blockPageAccentColorText");
        this.blockPageShowActionButtons = this.$("blockPageShowActionButtons");
        ...
        this.grayscaleToggle = this.$("grayscaleToggle");
    ...
    bindEvents() {
        ...
        if (this.importRulesBtn) {
            this.importRulesBtn.addEventListener("click", async () => this.importFileInput.click());
        }
        ...
        this.ruleType.addEventListener("change", () => {
            ...
            if (this.paramKeyInput)
                this.paramKeyInput.value = "";
            if (this.paramValueInput)
                this.paramValueInput.value = "";
            if (this.urlInput)
                this.urlInput.value = "";
        });
        ...
        if (this.blockPageMessage) {
            this.blockPageMessage.addEventListener("input", () => this.updatePreview());
        }
        if (this.blockPageShowQuotes) {
            this.blockPageShowQuotes.addEventListener("change", () => this.updatePreview());
        }
        if (this.useBackgroundImageUrlBtn) {
            this.useBackgroundImageUrlBtn.addEventListener("click", () => this.toggleBackgroundImageUrlInput());
        }
        if (this.importBackgroundImageBtn) {
            this.importBackgroundImageBtn.addEventListener("click", () => this.blockPageImageFileInput?.click());
        }
        if (this.removeBackgroundImageBtn) {
            this.removeBackgroundImageBtn.addEventListener("click", () => this.clearBackgroundImage());
        }
        if (this.blockPageImageFileInput) {
            this.blockPageImageFileInput.addEventListener("change", (event) => this.importBackgroundImageFile(event));
        }
        if (this.confirmBackgroundImageUrlBtn) {
            this.confirmBackgroundImageUrlBtn.addEventListener("click", () => this.applyBackgroundImageUrl());
        }
        ...
        if (this.blockPagePrimaryColorDisplay) {
            this.blockPagePrimaryColorDisplay.addEventListener("dblclick", () => this.showColorCodeInput("primary"));
        }
        if (this.blockPagePrimaryColorText) {
            this.bindColorCodeInput(this.blockPagePrimaryColorText, this.blockPagePrimaryColor, this.blockPagePrimaryColorDisplay);
        }
        ...
        if (this.blockPageAccentColorDisplay) {
            this.blockPageAccentColorDisplay.addEventListener("dblclick", () => this.showColorCodeInput("accent"));
        }
        if (this.blockPageAccentColorText) {
            this.bindColorCodeInput(this.blockPageAccentColorText, this.blockPageAccentColor, this.blockPageAccentColorDisplay);
        }
        if (this.blockPageShowActionButtons) {
            this.blockPageShowActionButtons.addEventListener("change", () => this.updatePreview());
        }
        if (this.savePersonalizationBtn) {
            this.savePersonalizationBtn.addEventListener("click", () => this.savePersonalization());
        }
        if (this.resetPersonalizationBtn) {
            this.resetPersonalizationBtn.addEventListener("click", () => this.resetPersonalization());
        }
        // Block page settings navigation
        if (this.openBlockPageSettingsBtn) {
            this.openBlockPageSettingsBtn.addEventListener("click", () => this.showBlockPageSettings());
        }
        if (this.backToThemeBtn) {
            this.backToThemeBtn.addEventListener("click", () => this.showThemeMainView());
        }
        // Custom rule sets events
        if (this.createRuleSetBtn) {
            this.createRuleSetBtn.addEventListener("click", () => this.startCreatingRuleSet());
        }
        if (this.saveRuleSetBtn) {
            this.saveRuleSetBtn.addEventListener("click", () => this.saveNewRuleSet());
        }
        if (this.cancelRuleSetBtn) {
            this.cancelRuleSetBtn.addEventListener("click", () => this.cancelRuleSetCreation());
        }
        if (this.backToRuleSetListBtn) {
            this.backToRuleSetListBtn.addEventListener("click", () => this.showRulesMainView());
        }
        if (this.backToRuleSetListBtn2) {
            this.backToRuleSetListBtn2.addEventListener("click", () => this.showRulesMainView());
        }
        // Click on "Rules" section title to show All Rules view
        if (this.rulesSectionTitle) {
            this.rulesSectionTitle.addEventListener("click", () => this.showAllRulesView());
        }
        ...
        if (this.modeGrayscaleToggle) {
            this.modeGrayscaleToggle.addEventListener("change", () => this.saveModeSettings());
        }
        if (this.configureModeSettingsBtn) {
            this.configureModeSettingsBtn.addEventListener("click", () => this.showModeSettingsView());
        }
        // Mode features view events
        if (this.backFromModeSettingsBtn) {
            this.backFromModeSettingsBtn.addEventListener("click", () => this.showModeEditFromSettings());
        }
        if (this.modeGrayscaleToggleView) {
            this.modeGrayscaleToggleView.addEventListener("change", () => this.saveModeSettingsFromView());
        }
        // Sessions events
        ...
        if (this.clearAllRuleSetsBtn) {
            this.clearAllRuleSetsBtn.addEventListener("click", () => this.clearAllRuleSets());
        }
        // Stats tab navigation events
        if (this.viewSessionHistoryBtn) {
            this.viewSessionHistoryBtn.addEventListener("click", () => this.showSessionHistoryView());
        }
        if (this.backToStatsBtn) {
            this.backToStatsBtn.addEventListener("click", () => this.showStatsMainView());
        }
        ...
        document.addEventListener("click", (e) => {
            ...
            if (!target?.closest(".item-menu-wrap") &&
    ...
    async loadState() {
        try {
            ...
            const themeColorDefaults = this.getThemeBlockPageDefaults(this.currentTheme);
            ...
            if (this.isActive) {
                ...
                const activeMode = this.customModes.find((mode) => mode.id === this.activeRuleSetId);
    ...
    startTimerUpdates() {
        ...
        this.timerInterval = setInterval(() => {
            ...
            if (this.isActive && Date.now() - this.lastTimerUpdate > 5000) {
                console.warn("Timer updates stopped, attempting to restart background timer");
                this.ensureBackgroundTimerRunning();
            }
        }, 1000);
    }
    ...
    commitCurrentSessionTime() {
        ...
        if (currentSession.sessionFocusStartTime) {
            const focusElapsed = Math.floor((Date.now() - currentSession.sessionFocusStartTime) / 1000);
            ...
            const breakElapsed = Math.floor((Date.now() - currentSession.sessionPauseStartTime) / 1000);
    ...
    updateActiveSessionInHistory() {
        ...
        const detailsElement = document.querySelector(`.session-details[data-session-id="${this.currentSessionId}"]`);
        if (!detailsElement)
            return;
        // Calculate total time with current elapsed time
        let totalTime = (currentSession.focusedTime || 0) + (currentSession.breakTime || 0);
        const now = Date.now();
        if (this.isActive && currentSession.sessionFocusStartTime) {
            const currentFocusElapsed = Math.floor((now - currentSession.sessionFocusStartTime) / 1000);
            totalTime += currentFocusElapsed;
        }
        else if (!this.isActive && currentSession.sessionPauseStartTime) {
            const currentBreakElapsed = Math.floor((now - currentSession.sessionPauseStartTime) / 1000);
            totalTime += currentBreakElapsed;
        }
        const ruleSet = this.customModes.find((rs) => rs.id === currentSession.ruleSetId);
        const ruleSetName = ruleSet ? ruleSet.name : "No Mode";
        detailsElement.textContent = this.formatSessionDetailsText(ruleSetName, currentSession.blocksCount || 0, totalTime);
    }
            ...
            if (this.currentSessionInfo &&
                ...
                if (totalLength > maxTotalLength) {
                    ...
                    if (totalLength > maxTotalLength) {
                        ...
                        const availableForRules = maxTotalLength -
                            staticLength -
                            sessionName.length -
                            blocksCount.length;
                        if (rulesName.length > availableForRules) {
                            ...
                            if (maxRulesChars > 0) {
                                ...
                                rulesName = rulesName.substring(0, Math.max(0, availableForRules));
                            }
                        }
                    }
                }
        ...
        try {
            ...
            let totalFocusedTime = this.sessions.reduce((sum, s) => {
                ...
                if (s.id === this.currentSessionId &&
                    ...
                    const currentFocusElapsed = Math.floor((Date.now() - s.sessionFocusStartTime) / 1000);
    ...
    async saveState(action, oldData) {
        try {
            ...
            if ((this.isActive || action === "toggleProducing") &&
                action !== "clearInfo" &&
                action !== "modeSettings") {
                const beforeRules = this.getRulesFromData(before.customModes, before.activeRuleSetId);
                chrome.runtime.sendMessage({
    ...
    async createNewSession() {
        ...
        const defaultName = `Session ${sessionDate.toLocaleDateString()} ${sessionDate.toLocaleTimeString([], { hour: "2-digit", minute: "2-digit" })}`;
    ...
    async toggleProducing() {
        ...
        if (this.isActive) {
            ...
            if (!activeMode) {
                this.isActive = false;
                this.showNotification("No mode active. Create and activate a mode for Producer to work.", "error");
                return;
            }
        ...
        this.updateUI();
        // Show feedback
        this.showNotification(this.isActive ? "Focus activated!" : "Focus deactivated");
    }
    updateUI() {
        ...
        if (this.currentSessionBlocksCount) {
            this.currentSessionBlocksCount.textContent = String(this.sessionBlocks || 0);
        }
        // Update stats in stats tab - Total Blocks across all sessions
        if (this.totalBlockedCountEl) {
            const totalBlocks = this.sessions.reduce((sum, s) => sum + (s.blocksCount || 0), 0);
    ...
    addRule() {
        ...
        if ((!url || !this.isValidUrl(this.cleanUrl(url))) &&
        ...
        if (cleanUrl === "*" && (type === "domain" || type === "url")) {
            ...
            if (this.isCreatingNewRuleSet && this.tempRuleSet) {
                ...
                ruleSet = this.customModes.find((rs) => rs.id === this.currentEditingRuleSetId);
            }
            if (ruleSet) {
                // Check if a wildcard rule already exists (in either domain or url type)
                const wildcardExists = ruleSet.rules.some((rule) => rule.url === "*" && (rule.type === "domain" || rule.type === "url"));
                if (wildcardExists) {
                    this.showNotification("'Block All Websites' rule already exists", "error");
        ...
        if (type === "allowParam") {
            ...
            if (!paramKey) {
                ...
                if (this.paramKeyInput)
                    this.paramKeyInput.value = ""; // Clear input
                if (this.paramValueInput)
                    this.paramValueInput.value = ""; // Clear input
                return;
            }
        }
        ...
        if (type !== "allowParam") {
            ...
            const existingRule = ruleSet.rules.find((rule) => rule.type !== "allowParam" && rule.url === cleanUrl);
            if (existingRule) {
                const existingTypeLabel = this.formatRuleType(existingRule);
                this.showNotification(`URL already added as "${existingTypeLabel}"`, "error");
                this.urlInput.value = ""; // Clear input
                return;
            }
        }
            ...
            const exists = ruleSet.rules.some((rule) => rule.type === "allowParam" &&
    ...
    editRuleUrlInline(rule, urlElement) {
        ...
        if (rule.type === "allowParam") {
            this.showNotification("Parameter rules cannot be edited. Please delete and re-add.", "error");
            return;
        }
    ...
    renderRulesList() {
      ...
      `;
        ...
        [...ruleSet.rules].reverse().forEach((rule) => {
            ...
            if (rule.permanent)
                item.classList.add("rule-permanent");
    ...
    formatSessionDetailsText(ruleSetName, blocksCount, totalTime) {
        ...
        const detailsText = `${shortRuleSetName} \u2022 ${blocksCount} block${blocksCount !== 1 ? "s" : ""} \u2022 ${hours}h${minutes}m`;
    ...
    async importRules(file) {
        ...
        try {
            ...
            if (importedRules.length === 0) {
                this.showNotification(skippedCount > 0
                    ? "No valid rules found in file"
                    : "No rules found in file", "error");
                return;
            }
            ...
            if (skippedCount > 0) {
                this.showNotification(`Imported ${importedRules.length} rule(s); skipped ${skippedCount} invalid`, "success");
            }
            else {
    ...
    exportRules() {
        ...
        const lines = ruleSet.rules.map((rule) => rule.type === "allowParam"
            ? `${rule.type} ${rule.paramKey}=${rule.paramValue}`
            : `${rule.type} ${rule.url}`);
        const fileContent = "# Producer Rules File\n" +
    ...
    getActiveRules() {
        if (!this.activeRuleSetId)
            return [];
        const ruleSet = this.customModes.find((rs) => rs.id === this.activeRuleSetId);
        return ruleSet ? ruleSet.rules : [];
    }
    getRulesFromData(customModes, activeRuleSetId) {
        if (!activeRuleSetId || !customModes)
            return [];
    ...
    toggleRulePermanent(ruleId, ruleSetId) {
        ...
        if (!rule || (rule.type !== "domain" && rule.type !== "url"))
            return;
        ...
        this.renderRulesList();
        this.renderAllRulesList();
        this.showNotification(newPermanent ? "Rule is now permanent" : "Rule is no longer permanent");
    }
    // Custom rule set management methods
    clearAllInputs() {
        // Clear rule set name
        if (this.ruleSetNameInput)
            this.ruleSetNameInput.value = "";
        // Clear URL input
        if (this.urlInput)
            this.urlInput.value = "";
        // Reset rule type dropdown to default
        if (this.ruleType)
            this.ruleType.selectedIndex = 0;
    ...
    saveNewRuleSet() {
        ...
        const activationMessage = this.activeRuleSetId === this.customModes[this.customModes.length - 1].id
            ? " and selected"
            : "";
        this.showNotification(`Custom mode successfully saved${activationMessage}!`);
        this.showRulesMainView();
    }
    ...
    saveRuleSetName() {
        if (!this.currentEditingRuleSetId || !this.ruleSetNameInput)
            return;
        ...
        const ruleSet = this.customModes.find((rs) => rs.id === this.currentEditingRuleSetId);
        if (!ruleSet)
            return;
    ...
    showRulesMainView() {
        if (this.rulesMainView)
            this.rulesMainView.style.display = "block";
        if (this.rulesEditView)
            this.rulesEditView.style.display = "none";
        if (this.allRulesView)
            this.allRulesView.style.display = "none";
        if (this.modeSettingsView)
            this.modeSettingsView.style.display = "none";
        this.currentEditingRuleSetId = null;
        this.isCreatingNewRuleSet = false;
        this.tempRuleSet = null;
    ...
    showAllRulesView() {
        ...
        if (this.rulesMainView)
            this.rulesMainView.style.display = "none";
        if (this.rulesEditView)
            this.rulesEditView.style.display = "none";
        if (this.allRulesView)
            this.allRulesView.style.display = "block";
        if (this.modeSettingsView)
            this.modeSettingsView.style.display = "none";
        this.renderAllRulesList();
        this.resetScrollPosition(document.getElementById("tab-rules"));
    ...
    renderAllRulesList() {
      ...
      `;
            ...
            if (this.addRuleAllViewBtn)
                this.addRuleAllViewBtn.style.display = "none";
            if (this.exportRulesAllViewBtn)
    ...
    showRulesEditView() {
        ...
        if (this.rulesMainView)
            this.rulesMainView.style.display = "none";
        if (this.rulesEditView)
            this.rulesEditView.style.display = "block";
        if (this.allRulesView)
            this.allRulesView.style.display = "none";
        if (this.modeSettingsView)
            this.modeSettingsView.style.display = "none";
    ...
    showModeSettingsView() {
        ...
        if (this.allRulesView)
            this.allRulesView.style.display = "none";
        if (this.modeSettingsView)
            this.modeSettingsView.style.display = "block";
    ...
    renderRulesPreview() {
        if (!this.rulesPreview || !this.rulesPreviewCount)
            return;
        let ruleSet;
        if (this.isCreatingNewRuleSet && this.tempRuleSet) {
            ruleSet = this.tempRuleSet;
      ...
      `;
        ...
        const ruleText = lastRule.type === "allowParam"
    ...
    renderRuleSetsList() {
      ...
      `;
        ...
        this.customModes.forEach((ruleSet, index) => {
            ...
            name.addEventListener("dblclick", (e) => {
                ...
                const saveEdit = () => {
                    ...
                    if (!newName) {
                        ...
                        const nameExists = this.customModes.some((rs) => rs.id !== ruleSet.id && rs.name === newName);
                        if (nameExists) {
                            this.showNotification("A mode with this name already exists", "error");
                        }
                        else {
            ...
            details.textContent = `${ruleSet.rules.length} rule${ruleSet.rules.length !== 1 ? "s" : ""}`;
    ...
    renderSessionHistory() {
        ...
        if (totalSessions > 0) {
            const totalFocusedTime = this.sessions.reduce((sum, s) => sum + (s.focusedTime || 0), 0);
            const avgTime = totalFocusedTime / totalSessions;
            const hours = Math.floor(avgTime / 3600);
            const minutes = Math.floor((avgTime % 3600) / 60);
            const avgString = `${hours.toString().padStart(2, "0")}h${minutes
      ...
      `;
        ...
        this.sessions.forEach((session, index) => {
            ...
            name.addEventListener("dblclick", (e) => {
                ...
                const saveEdit = () => {
                    ...
                    if (!newName) {
                        ...
                        const nameExists = this.sessions.some((s) => s.id !== session.id && s.name === newName);
                        if (nameExists) {
                            this.showNotification("A session with this name already exists", "error");
                        }
                        else {
            ...
            const ruleSet = this.customModes.find((rs) => rs.id === session.ruleSetId);
            const ruleSetName = ruleSet ? ruleSet.name : "No Mode";
            ...
            if (session.id === this.currentSessionId) {
                ...
                if (this.isActive && session.sessionFocusStartTime) {
                    const currentFocusElapsed = Math.floor((now - session.sessionFocusStartTime) / 1000);
                    totalTime += currentFocusElapsed;
                }
                else if (!this.isActive && session.sessionPauseStartTime) {
                    const currentBreakElapsed = Math.floor((now - session.sessionPauseStartTime) / 1000);
                    totalTime += currentBreakElapsed;
                }
            }
            details.textContent = this.formatSessionDetailsText(ruleSetName, session.blocksCount || 0, totalTime);
            info.appendChild(name);
            info.appendChild(details);
            // Drag handle (left)
            const dragHandle = document.createElement("span");
            ...
            actionsWrap.style.alignItems = "center";
            actionsWrap.style.flexShrink = "0";
            const hasStats = (session.focusedTime || 0) > 0 ||
                (session.breakTime || 0) > 0 ||
                (session.blocksCount || 0) > 0;
    ...
    clearSessions() {
        ...
        this._showConfirmModal({
            ...
            message: "All sessions and their data will be permanently deleted. This cannot be undone.",
            confirmText: "Clear All",
            onConfirm: () => {
    ...
    commitColorCodeInput(input, colorInput, display) {
        ...
        this.updatePreview();
        if (!isValidColor) {
            this.showNotification("Invalid color code. Use a valid hex value.", "error");
        }
    }
    ...
    refreshBackgroundImageControls() {
        ...
        this.setBackgroundImageStatus(this.getBackgroundImageLabel(imageValue, this.currentBlockPageBackgroundImageName));
        if (this.removeBackgroundImageBtn) {
    ...
    async importBackgroundImageFile(event) {
        ...
        if (file.size > maxImportBytes) {
            this.setBackgroundImageStatus("Image too large. Choose an image under 2 MB.");
        ...
        try {
            ...
            if (typeof dataUrl === "string" && this.blockPageImage) {
                const quotaBytes = chrome.storage?.local?.QUOTA_BYTES || 5 * 1024 * 1024;
                const bytesInUse = await chrome.storage.local.getBytesInUse(null);
                const previousImageBytes = this.estimateImageStorageBytes(this.blockPageImage.value || "", this.currentBlockPageBackgroundImageName || "");
                const newImageBytes = this.estimateImageStorageBytes(dataUrl, file.name);
                const projectedBytes = bytesInUse - previousImageBytes + newImageBytes;
                if (projectedBytes > quotaBytes) {
                    this.setBackgroundImageStatus("Image too large for extension storage. Use a smaller file.");
                    this.showNotification("Image exceeds storage limit. Pick a smaller image.", "error");
                    return;
                }
    ...
    getBlockPageSettingsFromInputs() {
        ...
        return {
            ...
            primaryColor: this.sanitizeHexColor(this.blockPagePrimaryColorText?.value?.trim(), defaults.primaryColor),
            accentColor: this.sanitizeHexColor(this.blockPageAccentColorText?.value?.trim(), defaults.accentColor),
            showActionButtons: this.blockPageShowActionButtons?.checked ?? defaults.showActionButtons,
        };
    }
    ...
    async selectTheme(theme) {
        ...
        try {
            ...
            chrome.runtime.sendMessage(this.getPersonalizationPayload({
    ...
    updatePreview() {
        ...
        if (this.blockPagePreview) {
            this.blockPagePreview.style.setProperty("--preview-background", this.getPreviewBackground(settings.primaryColor || defaults.primaryColor, settings.backgroundImage));
        }
    ...
    async savePersonalization(silent = false) {
        try {
            ...
            await chrome.storage.local.set({
                ...
                blockPageUseThemeColors: settings.primaryColor ===
    ...
    resetPersonalization() {
        const defaults = this.getDefaultBlockPageSettings();
        const alreadyDefault = this.currentBlockPageTitle === defaults.title &&
    ...
    async broadcastGrayscaleState(enabled = null) {
        ...
        try {
            ...
            console.error("[Producer Popup] Error broadcasting grayscale state:", error);
        }
    }
    ...
    async activateSession(sessionId) {
        ...
        const beforeRules = this.getRulesFromData(this.customModes, wasFocusModeActive ? session.ruleSetId : null);
    ...
    editSessionRuleSet(sessionId) {
        ...
        const currentRuleSet = this.customModes.find((rs) => rs.id === session.ruleSetId);
        ...
        const newRuleSet = this.customModes.find((rs) => rs.id === session.ruleSetId);
...
chrome.runtime.onMessage.addListener((message, sender, sendResponse) => {
    if (message.action === "updateBlockCount") {
        ...
        if (popup.currentSessionId) {
            const currentSession = popup.sessions.find((s) => s.id === popup.currentSessionId);
            if (currentSession) {
                // Update session data
                currentSession.blocksCount = popup.sessionBlocks;
                // Update Current Session section blocks count
                if (popup.currentSessionBlocksCount) {
                    popup.currentSessionBlocksCount.textContent = String(popup.sessionBlocks);
                }
